Program lensing
  ...
  RA,DEC,e1,e2,de1,de2,z,err_z,snr_win,mag_y
  double precision,dimension(:),allocatable:: &
  objid,z2,zErr,zPhoto,zPhotoErr,type,ra2,dec2 &
       ,veldisp,ved,mabs_r
...
!------------------------------------------------------------
  ...
  allocate( vdinf(dim1), vdsup(dim1), maginf(dim2),
  magsup(dim2), fich_sortie(dim1) ) 

!------------------------------------------------------------
  open(10,file='/home/amarchal/file_NGVS.txt',status='old', &
  ...
  open(11,file='/home/amarchal/file_SDSS.txt',status='old', &
       action='read',iostat=ios)
    if(ios .ne. 0)stop"Pb !"
    do i=1,116
  ...
  open(12,file='/home/amarchal/file_disp.txt',status='old', &
       action='read',iostat=ios)
     if(ios .ne. 0)stop"Pb !"
     read(12,*)
     do i=1,dim1
  ...
  open(13,file='/home/amarchal/file_mag.txt',status='old', &
       action='read',iostat=ios)
     if(ios .ne. 0)stop"Pb !"
     read(13,*)
     do i=1,dim2
  ...
  open(14,file='/home/amarchal/file_sortie.txt',status='old' &
       ,action='read',iostat=ios)
     if(ios .ne. 0)stop"Pb !"
     read(14,*)
     do i=1,dim1
...
!-------------------------------------------------------------
     do l=1,116
        call lecture_NGVS('/home/amarchal/NGVS_data/'//NGVS_maille(l), &
        nlNGVS,RA,DEC,e1,e2,de1,de2,z,err_z,snr_win,mag_y)
        call lecture_SDSS('/home/amarchal/new_SDSS_data/'//SDSS_maille(l), &
        nlSDSS,objid,z2,zErr,zPhoto,zPhotoErr,type,ra2,dec2,veldisp,ved,mabs_r)
!-----------------------calcul de sigmaS----------------------
        do i=1,nlNGVS-1  
...
!   wiso = 1. / sigma/sigma 
        do j=1,nlSDSS-1   
           !if (z2(j) .gt. 0 .and. z2(j) .lt. 0.5 .and. veldisp(j) .gt. 0. &
           !.and. veldisp(j) .lt. 180. ) then 
           !if (z2(j) .gt. 0.05 .and. z2(j) .lt. 0.6 .and. veldisp(j) .gt. vdinf(m)  &
           !.and. veldisp(j) .lt. vdsup(m)) then 
            
           if (z2(j) .gt. 0.05 .and. z2(j) .lt. 0.6 .and. mabs_r(j) &
                .gt. magsup(m) .and. mabs_r(j) .lt. maginf(m) .and. &
                veldisp(j) .gt. 10. .and. veldisp(j) .lt. 500.) then 

              do i=1,nlNGVS
                 if (z(i)+err_z(i) .gt. 0.7 .and. e1(i) .ne. 0 .and. e2(i) &
                      .ne. 0 .and. e1(i) .gt. -1 .and. e1(i) .lt. 1) then 
...
!                    B = max((ds-dl),0.)/ds
                    ...
                    x=0.; y=0.; r=0.; k=0.; et=0.; er=0.; alpha=0.; &
                    lr=0.; dl=0.; ds=0.; B=0.; B2=0; wiso=0.
                 endif
              enddo
...
!     ErrT(i)=0.26331349542305638/sqrt(N(i))!*sumB(i))   !A vérifier
        ...
        write(*,'(30(x,F15.6))') r2(i)*60,shearT(i),shearR(i), &
             1./sqrt(sumB(i)),N(i)
     enddo
!-----------------------Resultats----------------------
     ...
     do i=1,dim
        write(14,'(30(x,F15.6))') r2(i)*60,shearT(i),shearR(i), &
             1./sqrt(sumB(i)),N(i)
     enddo
     close(14) 
!-------------------------------------------------------------
...
!-------------Ecriture resultat vitesses de dispersions-------
     open(15,file='test/resultat.txt',status='old',position='append', &
